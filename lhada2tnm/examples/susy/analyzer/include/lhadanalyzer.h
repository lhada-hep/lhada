#ifndef LHADANALYZER_H
#define LHADANALYZER_H
//---------------------------------------------------------------------------
// File:        lhadanalyzer.h
...
// functions
inline
double	_dphijNjle3METmin(std::vector<TEParticle>& jetsSR, TLorentzVector& MET)
{
  std::vector<TLorentzVector> jetsSR_(jetsSR.size());
  copy(jetsSR.begin(), jetsSR.begin(), jetsSR_.begin());
  return dphijNjle3METmin(jetsSR_, MET);
};
  
  inline
  double	_aplanarity(std::vector<TEParticle>& jetsSR)
  {
    std::vector<TLorentzVector> jetsSR_(jetsSR.size());
    copy(jetsSR.begin(), jetsSR.begin(), jetsSR_.begin());
  ...
  inline
  double	_dphijNjgt3METmin(std::vector<TEParticle>& jetsSR, TLorentzVector& MET)
  {
    std::vector<TLorentzVector> jetsSR_(jetsSR.size());
    copy(jetsSR.begin(), jetsSR.begin(), jetsSR_.begin());
  ...
  inline
  double	_Meff(std::vector<TEParticle>& jetsSR, TLorentzVector& MET)
  {
    std::vector<TLorentzVector> jetsSR_(jetsSR.size());
    copy(jetsSR.begin(), jetsSR.begin(), jetsSR_.begin());
  ...
  inline
  double	_METovermeffNJ(std::vector<TEParticle>& jetsSR, int njets, TLorentzVector& MET)
  {
    std::vector<TLorentzVector> jetsSR_(jetsSR.size());
    copy(jetsSR.begin(), jetsSR.begin(), jetsSR_.begin());
  ...
  inline
  double	_METoversqrtHT(std::vector<TEParticle>& jetsSR, TLorentzVector& MET)
  {
    std::vector<TLorentzVector> jetsSR_(jetsSR.size());
    copy(jetsSR.begin(), jetsSR.begin(), jetsSR_.begin());
  ...
  // internal objects
  std::vector<TEParticle> muons;
  std::vector<TEParticle> jets;
  
  TEParticle MET;
  
  std::vector<TEParticle> electrons;
  std::vector<TEParticle> bjets;
  std::vector<TEParticle> cleanjets;
  std::vector<TEParticle> jetsSR;
  std::vector<TEParticle> cleanmuons;
  std::vector<TEParticle> cleanelectrons;
  std::vector<TEParticle> verycleanelectrons;
  ...
  // object_electrons_s object_electrons;
  ...
  //   };
  // };
  // object_cleanjets_s object_cleanjets;
  ...
  //   };
  // };
  // object_cleanmuons_s object_cleanmuons;
  ...
  //   };
  // };
  // object_cleanelectrons_s object_cleanelectrons;
  ...
  //   };
  // };
  // object_verycleanelectrons_s object_verycleanelectrons;
  ...
  struct cut_preselection_s : public lhadaThing
  {
    ...
    void count(std::string c)		{ hcount->Fill(c.c_str(), weight); }
    void write(TFile* out)	{ out->cd(); hcount->Write(); }
...
struct lhadanalyzer
{
  ...
  void analyze(std::vector<TEParticle>& Jet,
	       std::vector<TEParticle>& Electron,
	       std::vector<TEParticle>& Muon,
	       TEParticle& MissingET)
  {
    Delphes_Jet       = Jet;
    Delphes_Electron  = Electron;
    Delphes_Muon      = Muon;
    Delphes_MissingET = MissingET;
